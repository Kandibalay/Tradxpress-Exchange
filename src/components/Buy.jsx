    ...
    <div>
         <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-8'>
                <div className='flex flex-col w-full gap-2'>
                    <label htmlFor='crypto' className='font-bold text-[14px]'>Coin to Buy</label>
                    <select {...register('crypto', {required:'Select a Coin to Buy'})}
                    id='crypto' name='crypto' className='bg-white rounded-sm px-3 py-2 text-[14px] outline-none'>
                    ...
                    {errors.crypto && <p className='text-red-500 text-[12px] mx-0 text-right'>{errors.crypto.message}</p>}
                </div>
                <div className='flex flex-col w-full gap-2'>
